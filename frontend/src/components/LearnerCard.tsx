...
export default function LearnerCard({ learner, featured = false }: LearnerCardProps) {
  ...
  if (featured) {
    return (
      <Link
        ...
        className="group relative block bg-white/4 border border-white/10 rounded-2xl overflow-hidden hover:border-gold/35 transition-all duration-300 hover:-translate-y-0.5"
      >
        {isCertified && (
        ...
        <div className="p-6">
          ...
          {/* Bio as quote */}
          {learner.bio && (
            <p className="text-sm text-white/60 leading-relaxed line-clamp-3 mb-5 italic">
              {learner.bio}
            </p>
          )}
          ...
          <div className="flex items-center gap-3">
            {learner.photoUrl ? (
              <img
                ...
                className={`w-11 h-11 rounded-full object-cover border-2 flex-shrink-0 transition-colors ${
                  isCertified ? 'border-gold/40 group-hover:border-gold/70' : 'border-white/15 group-hover:border-white/30'
                }`}
              />
            ) : (
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                {learner.cohort && (
                  <span className="text-xs text-white/30">Cohorte {learner.cohort}</span>
                )}
            ...
            <ArrowRight className="w-4 h-4 text-white/20 group-hover:text-gold/60 transition-colors flex-shrink-0" />
          </div>
        </div>
      </Link>
    );
  ...
  return (
    <Link
      to={`/apprenants/${learner.slug}`}
      className="group block bg-white/4 border border-white/10 rounded-2xl overflow-hidden hover:border-gold/30 hover:bg-white/6 transition-all duration-300 hover:-translate-y-0.5"
    >
      <div className="p-5">
        {/* Avatar + nom */}
        <div className="flex items-start gap-3 mb-3">
          {learner.photoUrl ? (
            <img
              ...
              className={`w-12 h-12 rounded-full object-cover border-2 flex-shrink-0 transition-colors ${
                isCertified ? 'border-gold/40 group-hover:border-gold/60' : 'border-white/10 group-hover:border-white/25'
              }`}
            />
          ) : (
            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-base font-bold flex-shrink-0 ${
              isCertified ? 'bg-gold/15 border border-gold/25 text-gold' : 'bg-white/8 border border-white/10 text-white/50'
            }`}>
              {learner.firstName[0]}{learner.lastName[0]}
            </div>
          )}
          ...
          <div className="min-w-0 flex-1">
            ...
            {learner.cohort && (
              <p className="text-xs text-white/35 mt-0.5">Cohorte {learner.cohort}</p>
            )}
        ...
        {/* Bio */}
        {learner.bio && (
          <p className="text-xs text-white/45 line-clamp-2 mb-3 leading-relaxed">{learner.bio}</p>
        )}
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-white/6">
            {learner.linkedinUrl && (
              <a
                ...
                className="flex items-center gap-1.5 text-xs text-white/35 hover:text-blue-400 transition-colors"
              >
            ...
            {learner.portfolioUrl && (
              <a
                ...
                rel="noopener noreferrer"
                onClick={e => e.stopPropagation()}
                className="flex items-center gap-1.5 text-xs text-white/35 hover:text-gold transition-colors"
              >
