...
import { ArrowRight, BookOpen, CalendarDays, ChevronDown, Info, LogOut, Menu, Shield, User } from 'lucide-react';
...
export default function Header() {
  ...
  const accountActions = (
    <div className="space-y-2">
      <Link to={isAdmin ? '/admin' : '/profil'} className="flex min-h-12 items-center gap-3 rounded-xl bg-white/5 px-4 text-sm text-t1 transition-colors hover:bg-white/10">
        {isAdmin ? <Shield className="h-4 w-4 text-gold" aria-hidden /> : <User className="h-4 w-4 text-gold" aria-hidden />}
        {isAdmin ? 'Ouvrir l’administration' : 'Mon espace personnel'}
        <ArrowRight className="ml-auto h-4 w-4" aria-hidden />
      </Link>
      <button type="button" disabled={leaving} onClick={() => void logout()} className="flex min-h-12 w-full items-center gap-3 rounded-xl px-4 text-sm text-t3 transition-colors hover:bg-white/5 hover:text-t1 disabled:opacity-50">
        <LogOut className="h-4 w-4" aria-hidden />{leaving ? 'Déconnexion…' : 'Se déconnecter'}
      </button>
      {failure && <p role="alert" className="px-4 text-sm text-error">{failure}</p>}
    </div>
  );
    ...
    <header className="sticky top-0 z-40 border-b border-line-soft bg-background/95 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center gap-8 px-5 sm:px-8">
          ...
          {isLoading ? <span role="status" className="h-10 w-28 animate-pulse rounded-full bg-white/5"><span className="sr-only">Vérification de la session…</span></span> : isSignedIn ? (
            <Dialog open={accountOpen} onOpenChange={setAccountOpen}>
              <DialogTrigger asChild>
                <button type="button" className="flex items-center gap-3 rounded-full border border-line py-1.5 pl-1.5 pr-4 text-sm text-t2 transition-colors hover:border-line-strong hover:text-t1">
                  ...
                  <span className="max-w-32 truncate">{isAdmin ? 'Administration' : 'Mon espace'}</span><ChevronDown className="h-3.5 w-3.5" aria-hidden />
                </button>
              </DialogTrigger>
              <DialogContent className="w-[calc(100%_-_2rem)] rounded-2xl border-line bg-card sm:max-w-sm sm:rounded-2xl">
                <DialogTitle className="pr-6 text-xl">{name || 'Mon compte'}</DialogTitle>
                <DialogDescription className="break-words">{email || (isAdmin ? 'Espace d’administration LesCracks' : 'Ton espace LesCracks')}</DialogDescription>
                {accountActions}
              </DialogContent>
            </Dialog>
          ) : (
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="top-4 w-[calc(100%_-_2rem)] translate-y-0 rounded-2xl border-line bg-card sm:rounded-2xl">
            <DialogTitle className="font-display text-xl">Explorer LesCracks</DialogTitle>
            <DialogDescription>Un sujet, une ressource, un prochain pas.</DialogDescription>
            <nav aria-label="Navigation mobile" className="my-2 space-y-2">
              {LINKS.map(({ to, label, icon: Icon, description }) => (
                <NavLink key={to} to={to} onClick={() => setOpen(false)} className={({ isActive }) => `flex items-center gap-4 rounded-xl p-4 transition-colors ${isActive ? 'bg-gold/10 text-gold' : 'bg-white/5 text-t1 hover:bg-white/10'}`}>
                  <Icon className="h-5 w-5 shrink-0" aria-hidden /><span><span className="block text-base font-medium">{label}</span><span className="mt-1 block text-xs text-t3">{description}</span></span><ArrowRight className="ml-auto h-4 w-4" aria-hidden />
                </NavLink>
              ))}
            </nav>
            <div className="border-t border-line-soft pt-4">
              {isLoading ? <p role="status" className="text-sm text-t3">Vérification de la session…</p> : isSignedIn ? accountActions : <div className="grid gap-3"><Link to="/inscription" state={destination} onClick={() => setOpen(false)} className="btn-primary">Créer un compte</Link><Link to="/connexion" state={destination} onClick={() => setOpen(false)} className="btn-secondary">Se connecter</Link></div>}
            </div>
          </DialogContent>
        </Dialog>
      </div>
