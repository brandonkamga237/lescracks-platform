...
export default function AdminDashboard() {
  ...
  return <AdminSection title="Vue d’ensemble" description={`Bonjour${name ? ` ${name}` : ''}. Voici l’état actuel de la plateforme.`} action={<div className="flex flex-wrap gap-3"><button type="button" onClick={() => { void overview.reload(); void growth.reload(); void top.reload(); }} disabled={overview.loading} className="btn-secondary">{overview.loading ? 'Actualisation…' : 'Actualiser'}</button><button type="button" onClick={() => void adminApi.exportUsers()} className="btn-secondary border-gold-400/30 text-gold-400 hover:bg-gold-400/10"><Download className="h-4 w-4" aria-hidden /> Utilisateurs CSV</button></div>}>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <div className="rounded-3xl border border-white/[0.06] bg-card p-6">
            <div className="flex items-center justify-between">
              ...
              <select value={growthDays} onChange={(e) => setGrowthDays(Number(e.target.value))} className="input h-11 w-auto px-4 py-0">
