    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center py-40 px-4 text-center">
          ...
          <h1 className="text-title text-white mb-2">Ressource introuvable</h1>
  ...
  return (
    <Layout>
      ...
      <div className="pt-20 pb-4 px-4 max-w-4xl mx-auto">
        <nav className="flex items-center gap-2 text-label text-t4">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-16">
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            ...
            >
              <span className={`px-2.5 py-1 text-label rounded-full font-medium ${
                isVideo ? 'bg-info-subtle text-info' : 'bg-gold/15 text-gold'
              }`}>
                {isVideo ? 'Vidéo' : 'Document'}
              </span>
              {resource.categoryName && (
                <span className="px-2.5 py-1 text-label rounded-full bg-white/6 text-t3">
                  {resource.categoryName}
                </span>
              )}
              {fileSize && (
                <span className="px-2.5 py-1 text-label rounded-full bg-white/6 text-t4">
                  {fileSize}
                </span>
              )}
            </motion.div>
            ...
            {resource.description && (
              <motion.p
                ...
                className="text-t2 leading-relaxed mb-6 text-body"
              >
                {resource.description}
              </motion.p>
            )}
              ...
              >
                ...
                {resource.tags.map(tag => (
                  <RouterLink
                    ...
                    className="px-2 py-0.5 text-label rounded bg-white/5 border border-line-soft text-t3 hover:text-t1 hover:border-line-strong transition-colors"
                  >
                    {tag.name}
                  </RouterLink>
                ))}
            ...
            {/* Stats */}
            <div className="flex items-center gap-5 text-label text-t4 mb-6">
            ...
            >
              {canAccess ? (
                <div className="space-y-3">
                  <p className="text-label text-t3 text-center mb-1">Accès libre</p>
                  <button
                    onClick={handleOpen}
                    className="w-full flex items-center justify-center gap-2 bg-gold text-black font-semibold px-4 py-3 rounded-xl hover:bg-gold/80 transition-colors text-body"
                  >
                    {isVideo ? (
                  ...
                  {resource.downloadable && !isVideo && (
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-white/6 border border-line text-t1 font-medium px-4 py-2.5 rounded-xl hover:bg-white/10 transition-colors text-body"
                    >
                      <Download className="w-4 h-4" /> Télécharger
                    </button>
                  )}
                  {downloadError && (
                    <p role="alert" className="text-label text-error text-center">{downloadError}</p>
                  )}
                </div>
              ) : (
